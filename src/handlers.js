...
const signIn = async (req, res) => {
  ...
  if (!findUser || (findUser.password.password !== password)) {
...
const signUp = async (req, res) => {
  ...
  await newUser.createPassword({password});
...
const tokenValidate = (req, res, next) => {
  if ((req.method === 'POST') && (req.url.includes('/api/signin'))) {
    next();
    return;
  }

  if ((req.method === 'POST') && (req.url.includes('/api/signup'))) {
    next();
    return;
  }
